...
var account = {
  getAccountDetails: function(req, res, next){
    ...
    var getAccountData = function(callback) {
      req.app.db.models.Account.findById(req.user.roles.account.id, 'name company phone zip purchaseHistoryLog').exec(function(err, account) {
        if (err) {
    ...
    var getPHData = function(callback) {
      req.app.db.models.PurchaseHistory.find({ "user._id" : req.user.id }, 'account.name').exec(function(err, purchasehistory) {
        if (err) {
          callback(err, null);
        }
    ...
    require('async').parallel([getAccountData, getUserData, getPHData], asyncFinally);
  },
